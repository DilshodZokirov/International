...
from configs.constants import PAYMENTS_PROVIDER_TOKEN
...
async def cmd_start(message: types.Message):
    await message.bot.send_message(chat_id=message.chat.id,
                                   text="Hello, I'm the demo merchant bot."
...
async def cmd_buy(message: types.Message):
    ...
    await message.bot.send_invoice(message.chat.id, title='Working Time Machine',
...
async def shipping(shipping_query: types.ShippingQuery):
    await shipping_query.bot.answer_shipping_query(shipping_query.id, ok=True, shipping_options=shipping_options,
...
async def checkout(pre_checkout_query: types.PreCheckoutQuery):
    await pre_checkout_query.bot.answer_pre_checkout_query(pre_checkout_query.id, ok=True,
                                                           error_message="Aliens tried to steal your card's CVV,"
                                                                         " but we successfully protected your credentials,"                                                      " try to pay again in a few minutes, we need a small rest.")
